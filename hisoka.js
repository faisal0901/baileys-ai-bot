...
const generateText = async (prompt) => {
  try {
      let yanz = await blackbox(prompt);
...
async function start() {
   ...
   hisoka.ev.on("messages.upsert", async (message) => {
      ...
      // console.log('send from',message.key.remoteJid);
    
      // console.log(text);
      // await  hisoka.sendMessage(message.key.remoteJid, { text: text });
      if (!m.fromMe && m.body.startsWith(".ai")) {
         const prompt = m.body.slice(4);
         const text = await generateText(prompt);
         console.log(text);
         const from =message.messages[0].key.remoteJid;
         await hisoka.sendMessage(from,{text:text});
       }
